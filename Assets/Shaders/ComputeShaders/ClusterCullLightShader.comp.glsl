...
//Shared variables 
shared uint sharedLights[16*9*4];
...
void main(){
    ...
    for( uint batch = 0; batch < numBatches; ++batch){
        ...
        sharedLights[gl_LocalInvocationIndex] = lightIndex;
        barrier();

        //Iterating within the current batch of lights
        for( uint light = 0; light < threadCount; ++light){
            if( pointLight[sharedLights[light]].enabled  == 1){
...
bool testSphereAABB(uint light, uint tile){
    float radius = pointLight[sharedLights[light]].range;
    vec3 center  = vec3(viewMatrix * pointLight[sharedLights[light]].position);
